...
class Weibo():
    ...
    def get_ana(self, request):
        uid = request.session.get("uid")
        weibos = self.get_user_follow_weibo(uid)
        weibos_my = self.get_user_weibo(uid)
        weibos = weibos + weibos_my

        weibos = sorted([i["weibo_date"] for i  in weibos])
        weibos = [i[:10] for i in weibos]
        weibo_time = dict()
        for i in weibos:
            if i in weibo_time:
                weibo_time[i] += 1
                ...
                weibo_time[i] = 1
        weibo_time = sorted(weibo_time.items(), key=lambda v:v[0], reverse=False)
        weibo_time_final = []
        for i,j in weibo_time:
            i = i+"T01:00:00"
            weibo_time_final.append([i,j])
        weibos = [ [time.mktime(datetime.datetime.strptime(i, "%Y-%m-%dT%H:%M:%S").timetuple())*1000,j ] for i,j in weibo_time_final]
        context_dict = dict()
        context_dict["data"] = str(weibos)
        return render(request, 'myapp/vis.html', context_dict)
